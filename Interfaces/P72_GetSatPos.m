...
OMEGA_k = Data.Omega_0 * pi +(Data.DOmega * pi - OMEGAdot_e) * t_k - ...
                                                   OMEGAdot_e  * Data.t_oe;
satX = x_k * cos(OMEGA_k) - y_k * cos(i_k) * sin(OMEGA_k);
satY = x_k * sin(OMEGA_k) + y_k * cos(i_k) * cos(OMEGA_k);
satZ = y_k * sin(i_k);

SatPos = [satX; satY; satZ; x_k; y_k; i_k; OMEGA_k];
% time correction 
tc = t - Data.t_oc;
delta_t_sv = Data.a_f0 + Data.a_f1 * tc + Data.a_f2 * tc ^ 2 - Data.T_GD;%* 1e-9;
% delta_t_r = F * s2.e * s2.sqrt_A * sin(E_k);
% t_corr = delta_t_sv + delta_t_r;
GPSTime = t - delta_t_sv;
